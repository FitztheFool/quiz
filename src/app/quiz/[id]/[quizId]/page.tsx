...
export default function QuizPage() {
    ...
    useEffect(() => {
        if (!quizId || !lobbyId || !session?.user?.id) return;
    ...
    }, [lobbyId, quizId, session?.user?.id, socket]);
    ...
    const submitQuiz = useCallback((answers: UserAnswer[]) => {
        ...
        router.push(`/quiz/${lobbyId}/${quizId}/result`);
    }, [session?.user?.id, lobbyId, socket, quizId, router]);
        ...
        return (
            <div className="flex items-center justify-center bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white py-16">
                <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-10 flex flex-col items-center gap-4 text-center max-w-sm w-full">
                    ...
                    <button onClick={() => router.push(`/lobby/create/${lobbyId}`)} className="mt-2 text-sm text-blue-600 dark:text-blue-400 hover:underline">Retour au lobby</button>
                </div>
            </div>
        );
    }
                    ...
                    {status === 'unauthenticated' && (
                        <div className="flex items-center gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 text-amber-800 dark:text-amber-300 rounded-xl px-4 py-3">
                            ...
                            <p className="text-sm">
                                ...
                                <Link href={`/login?callbackUrl=${encodeURIComponent(`/quiz/${lobbyId}/${quizId}`)}`} className="font-semibold underline hover:text-amber-900 dark:hover:text-amber-200 transition-colors">
                                    Se connecter
                                </Link>
                            </p>
                        </div>
