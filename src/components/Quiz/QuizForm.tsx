    ...
    function validateAnswers(qi: number, answers: AnswerForm[], type: QuestionType) {
        ...
        if (type === 'MCQ_UNIQUE') {
            if (answers.some(a => !a.text.trim())) msg = 'Une réponse est vide';
                ...
                msg = 'Coche exactement une bonne réponse';
        }
        ...
        for (const [qi, q] of form.questions.entries()) {
            ...
            if (q.type === 'MCQ_UNIQUE') {
                if (!q.answers || q.answers.length < 2)
                    return `Question ${qi + 1}: minimum 2 réponses.`;
                if (q.answers.some(a => !a.text.trim()))
                    return `Question ${qi + 1}: une réponse est vide.`;
                if (q.answers.filter(a => a.isCorrect).length !== 1)
                    return `Question ${qi + 1}: coche exactement une bonne réponse.`;
            }
        }
        return null;
    }
